...
void static inline generateSignal(const uint8_t *signal, uint8_t ad2, uint8_t ad1, uint8_t ad0)
{
...
void static inline generateSteps(uint8_t ad2, uint8_t ad1, uint8_t ad0){
/*
;	Dds1,Dds2,Dds3 are the phase accumulators
;  	ad2 ..  ad1 are the adder value determining frequency
;
; 	add value to accumulator
;	swap Dds2
;	output last bit  (higher nibble of Dds2) to port
;	repeat 	
*/
	static uint8_t dds1;
	static uint8_t dds2;
	static uint8_t dds3;
	dds1 = 0; dds2 = 0; dds3 = 0;
	dds1 = 
}

void startFreqGen(uint32_t freq){
	SG.acc=freq/RESOLUTION;
	SG.flag=1;//set flag to start generator
	SPCR&=~(1<<CPHA);//clear CPHA bit in SPCR register to allow DDS
	generateSignal(squarewave,
